...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteQueryCoversheet` (`p_id` INT)   BEGIN
	DELETE FROM coverreplies WHERE QueryID = p_id;
	DELETE FROM coversheetquery WHERE ID = p_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `DeleteQueryManuscript` (`p_id` INT)   BEGIN
	DELETE FROM queryreplies WHERE QueryID = p_id;
	DELETE FROM manuscriptquery WHERE ID = p_id;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllCoverReplies` (`p_query_id` INT)   BEGIN
	SELECT * FROM coverreplies 
    WHERE QueryID = p_query_id 
    ORDER BY ID DESC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllCoversheet` ()   BEGIN
SELECT * FROM jobtrackaunz.coversheet_mt;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllCoversheetData` (`p_filterBy` INT, `p_CodingDueDate` DATETIME, `p_OnlineDueDate` DATETIME, `p_OnlineTimeliness` VARCHAR(128))   BEGIN
	SELECT * FROM coversheetdata
    WHERE
		CASE 
			WHEN p_filterBy = 1 THEN CodingDueDate = p_CodingDueDate
            WHEN p_filterBy = 2 THEN OnlineDueDate = p_OnlineDueDate
            WHEN p_filterBy = 3 THEN OnlineTimeliness = p_OnlineTimeliness
            ELSE coversheetdata.CoversheetID > 0
		END;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllCoversheetDataByProductAndService` (`p_BPSProductID` VARCHAR(10), `p_ServiceNumber` VARCHAR(100))   BEGIN
	SELECT
		A.CoversheetID,
		A.CoversheetNumber,
		A.CoversheetTier,
    ...
    LEFT JOIN employee B ON B.ID = A.JobOwner
	WHERE A.BPSProductID = p_BPSProductID AND A.ServiceNumber = p_ServiceNumber;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllCoversheetDataByProductAndServiceByUser` (`p_BPSProductID` VARCHAR(10), `p_ServiceNumber` VARCHAR(100), `p_Username` VARCHAR(100))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobCoversheetData` ()   BEGIN
	...
	-- SELECT * FROM jobdata
	-- INNER JOIN jobcoversheetdata ON jobcoversheetdata.JobID = jobdata.JobID
	-- INNER JOIN coversheet_mt ON coversheet_mt.CoversheetID = jobcoversheetdata.JobCoversheetID;
    
    -- SELECT * FROM coversheetdata
	-- LEFT JOIN manuscriptdata ON manuscriptdata.ManuscriptID = coversheetdata.ManuscriptID
	-- LEFT JOIN jobdata ON jobdata.JobNumber = manuscriptdata.JobNumber;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobCoversheetDataByUserNameLEorPE` (`p_Username` VARCHAR(45))   BEGIN
	DECLARE p_EmployeeID INT;
	...
	LEFT JOIN TransactionLog B ON B.BPSProductID = A.BPSProductID AND B.ServiceNumber = A.ServiceNumber
	LEFT JOIN employee C ON C.UserName = B.ValueAfter OR C.UserName = B.ValueBefore
	WHERE C.ID = p_EmployeeID;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobCoversheetDataByUserNamePE` (`p_Username` VARCHAR(45))   BEGIN
    ...
    -- get employee id
	SELECT EmployeeID INTO p_EmployeeID FROM employee   
	WHERE Username = p_Username;
	...
	FROM JobCoversheetData A
	LEFT JOIN TransactionLog B ON B.BPSProductID = A.BPSProductID AND B.ServiceNumber = A.ServiceNumber
	LEFT JOIN employee C ON C.UserName = B.ValueAfter
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobDataByUserName` (`p_Username` NVARCHAR(128))   BEGIN
	DECLARE p_EmployeeID INT;
	...
	-- get jobdata based on employee id
	SELECT * FROM jobtrackaunz_v1.jobdata
	LEFT JOIN employee ON employee.ID = jobdata.CreatedEmployeeID
	WHERE employee.UserName = p_Username;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobDataByUserNameLE` (`p_Username` VARCHAR(128))   BEGIN
    DECLARE p_EmployeeID INT;
    ...
    SELECT 
		...
		, A.STPStatus
		, A.DateCreated
	FROM jobtrackaunz_v1.jobdata A
	LEFT JOIN jobtrackaunz_v1.transactionlog B ON B.BPSProductID = A.BPSProductID AND B.ServiceNumber = A.ServiceNumber
    LEFT JOIN jobtrackaunz_v1.employee C ON C.UserName = B.ValueAfter OR C.UserName = B.ValueBefore
	WHERE C.ID = p_EmployeeID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobDataByUserNamePE` (`p_Username` VARCHAR(128))   BEGIN
	DECLARE p_EmployeeID INT;
	...
	FROM jobtrackaunz_v1.jobdata A
	LEFT JOIN jobtrackaunz_v1.transactionlog B ON B.BPSProductID = A.BPSProductID AND B.ServiceNumber = A.ServiceNumber
    LEFT JOIN jobtrackaunz_v1.employee C ON C.UserName = B.ValueAfter
	WHERE C.ID = p_EmployeeID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobReassignment` ()   BEGIN
	SELECT
		D.TransactionLogID,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobReassignmentByUser` (`p_UserName` VARCHAR(128), `p_UserAccess` INT)   BEGIN

	-- LE
	IF p_UserAccess = 2		
    THEN
		SELECT * FROM transactionlog
		WHERE TransactionLogName = 'Manuscript' 
		AND TransactionType = 'JobReassignment LE'
		AND ValueAfter = p_UserName OR ValueBefore = p_UserName;
    
    -- PE
    ELSEIF p_UserAccess = 3
    THEN
		SELECT * FROM transactionlog
		WHERE TransactionLogName = 'Manuscript' 
		AND TransactionType = 'JobReassignment PE'
		AND ValueAfter = p_UserName OR ValueBefore = p_UserName;
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobsByProductAndServiceAndDueStatus` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250), `p_DueStatus` VARCHAR(128))   BEGIN
	SELECT COUNT(A.CoversheetID) as 'JobCount'
	FROM coversheetdata A 
    LEFT JOIN employee B ON B.ID = A.JobOwner
	WHERE 
        CASE 
			WHEN p_DueStatus = 'Late' THEN A.TargetPressDate < utc_timestamp() 
            WHEN p_DueStatus = 'Revised' THEN A.RevisedOnlineDueDate < utc_timestamp()
            WHEN p_DueStatus = 'DueIn5' THEN A.CodingDueDate > utc_timestamp() AND A.CodingDueDate < adddate(utc_timestamp(), INTERVAL 5 DAY)
			ELSE TargetPressDate = utc_timestamp() 
		END
		AND A.TaskStatus = 'New'
		AND find_in_set(A.BPSProductID, p_BPSProductIDs) 
		AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobsByProductAndServiceAndDueStatusLE` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250), `p_DueStatus` VARCHAR(128))   BEGIN
	SELECT COUNT(A.ManuscriptID) as 'JobCount'
	FROM manuscriptdata A 
    LEFT JOIN employee B ON B.ID = A.CreatedEmployeeID
    WHERE
		CASE 
			WHEN p_DueStatus = 'Late' THEN A.TargetPressDate < utc_timestamp() 
            WHEN p_DueStatus = 'CopyLate' THEN A.CopyEditDueDate < utc_timestamp() 
            WHEN p_DueStatus - 'CopyDue' THEN A.CopyEditDueDate = utc_timestamp()
            WHEN p_DueStatus = 'Revised' THEN A.RevisedOnlineDueDate < utc_timestamp()
            WHEN p_DueStatus = 'DueIn5' THEN A.CodingDueDate > utc_timestamp() AND A.CodingDueDate < adddate(utc_timestamp(), INTERVAL 5 DAY)
			ELSE TargetPressDate = utc_timestamp() 
		END
        AND A.CodingStatus = 'New'
		AND find_in_set(A.BPSProductID, p_BPSProductIDs) 
		AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobsByProductAndServiceAndDueStatusSTP` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250), `p_DueStatus` VARCHAR(128))   BEGIN
	SELECT COUNT(A.ID) as 'JobCount'
    FROM stp A
	-- FROM coversheetdata A 
    -- LEFT JOIN employee B ON B.ID = A.JobOwner
	WHERE 
        CASE 
			...
			ELSE TargetPressDate = utc_timestamp() 
		END
		AND A.SendToPrintStatus = 'New'
		AND find_in_set(A.BPSProductID, p_BPSProductIDs) 
		AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobsByProductAndServiceAndStatus` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250), `p_Status` VARCHAR(128))   BEGIN
	SELECT COUNT(A.CoversheetID) as 'JobCount'
	FROM coversheetdata A 
    LEFT JOIN employee B ON B.ID = A.JobOwner
	WHERE 
		A.TaskStatus = p_Status
		AND find_in_set(A.BPSProductID, p_BPSProductIDs) 
		AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobsByProductAndServiceAndStatusLE` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250), `p_Status` VARCHAR(128))   BEGIN
	SELECT COUNT(A.ManuscriptID) as 'JobCount'
	FROM manuscriptdata A 
    LEFT JOIN employee B ON B.ID = A.CreatedEmployeeID
	WHERE 
        CASE 
			WHEN p_Status = 'New' THEN A.CodingStatus = p_Status OR A.CodingStatus = 'On-Going' 
            ELSE A.CodingStatus = p_Status
        END
		AND find_in_set(A.BPSProductID, p_BPSProductIDs) 
		AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllJobsByProductAndServiceAndStatusSTP` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250), `p_Status` VARCHAR(128))   BEGIN
	SELECT COUNT(A.ID) as 'JobCount'
	FROM stp A 
    -- LEFT JOIN employee B ON B.ID = A.JobOwner
	WHERE 
		A.SendToPrintStatus = p_Status
		AND find_in_set(A.BPSProductID, p_BPSProductIDs) 
		AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllLateJobsByProductAndService` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250))   BEGIN
	SELECT COUNT(A.CoversheetID) as 'JobCount'
	FROM coversheetdata A 
    LEFT JOIN employee B ON B.ID = A.JobOwner
	WHERE 
		A.TaskStatus = 'New' 
        AND A.CodingDueDate < utc_timestamp()
		AND find_in_set(A.BPSProductID, p_BPSProductIDs) 
		AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllManuscript` ()   BEGIN
SELECT 
*
 FROM Manuscript;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllManuscriptData` (`p_filterBy` INT, `p_BPSProductID` VARCHAR(128), `p_ServiceNumber` VARCHAR(128), `p_UpdateType` VARCHAR(128), `p_CopyEditDueDate` DATETIME)   BEGIN
	SELECT * FROM Manuscriptdata
    WHERE
		CASE 
			WHEN p_filterBy = 1 THEN BPSProductID = p_BPSProductID
            WHEN p_filterBy = 2 THEN ServiceNumber = p_ServiceNumber
            WHEN p_filterBy = 3 THEN UpdateType = p_UpdateType
            WHEN p_filterBy = 4 THEN CopyEditDueDate = p_CopyEditDueDate
            ELSE Manuscriptdata.ManuscriptID > 0
		END;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllMyJobsByProductAndService` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250))   BEGIN
	SELECT COUNT(A.CoversheetID) as 'JobCount'
	FROM coversheetdata A 
    LEFT JOIN employee B ON B.ID = A.JobOwner
	WHERE find_in_set(A.BPSProductID, p_BPSProductIDs) AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllMyJobsByProductAndServiceLE` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250))   BEGIN
	SELECT COUNT(A.ManuscriptID) as 'JobCount'
	FROM manuscriptdata A 
    LEFT JOIN employee B ON B.ID = A.CreatedEmployeeID
	WHERE find_in_set(A.BPSProductID, p_BPSProductIDs) AND find_in_set(A.ServiceNumber, p_ServiceNumbers);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllMyJobsByProductAndServiceSTP` (`p_BPSProductIDs` VARCHAR(250), `p_ServiceNumbers` VARCHAR(250))   BEGIN
	SELECT COUNT(A.ID) as 'JobCount'
    FROM stp A
	-- FROM coversheetdata A 
    -- LEFT JOIN employee B ON B.ID = A.JobOwner
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllProductAndServiceByUsername` (`p_Username` VARCHAR(128))   BEGIN
	DECLARE p_EmployeeID INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllProductAndServiceByUsernameLE` (`p_Username` VARCHAR(128))   BEGIN
	DECLARE p_EmployeeID INT;
    ...
    SELECT  A.BPSProductID, A.ServiceNumber
	FROM jobdata A
	LEFT JOIN transactionlog B ON B.BPSProductID = A.BPSProductID AND B.ServiceNumber = A.ServiceNumber
    LEFT JOIN employee C ON C.UserName = B.ValueAfter
	WHERE C.ID = p_EmployeeID;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllProductAndServiceByUsernameSTP` (`p_Username` VARCHAR(128))   BEGIN
	DECLARE p_EmployeeID INT;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllPublicationAssignment` ()   BEGIN
	SELECT * FROM publicationassignment ORDER BY PublicationAssignmentID DESC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllPublicationAssignmentByRole` (`p_UserAccess` INT)   BEGIN
	IF p_UserAccess = 2		-- LE
    THEN
		SELECT DISTINCT LEUserName from publicationassignment;
        
    ELSEIF p_UserAccess = 3		-- PE
    THEN
		SELECT DISTINCT PEUserName from publicationassignment;
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllPubschedBPSProductID` ()   BEGIN
	SELECT DISTINCT BPSProductID 
    FROM pubsched_mt
	ORDER BY bpsproductid ASC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllPubschedBPSProductIDByLE` (`p_UserName` NVARCHAR(128))   BEGIN
	SELECT DISTINCT BPSProductID
    FROM publicationassignment
    WHERE LEUserName = p_UserName;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllPubschedByProductAndService` (`p_BPSProductID` VARCHAR(10), `p_ServiceNumber` VARCHAR(100))   BEGIN
	SELECT * FROM pubsched_mt
    WHERE BPSProductID = p_BPSProductID AND ServiceNumber = p_ServiceNumber;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllPubschedServiceNumber` (`p_BPSProductID` VARCHAR(10), `p_ServiceNumber` VARCHAR(100))   BEGIN
	IF (p_ServiceNumber IS NULL) 
    THEN
		SELECT 
			PubSchedTier
			, bpsproductid
            , servicenumber
            , CASE WHEN budgetpressdate <> '1900-01-01' THEN budgetpressdate ELSE revisedpressdate END AS TargetPressDate
	...
	/* IF (p_ServiceNumber IS NOT NULL) 
    THEN
		SELECT 
			PubSchedTier
            , bpsproductid
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllQueryCoversheet` (`p_coversheet_id` INT, `p_filterBy` INT)   BEGIN
	DECLARE p_limit int;
	SELECT COUNT(ID) FROM coversheetquery WHERE CoversheetID = p_coversheet_id INTO p_limit;
    
	SELECT 
		coversheetquery.ID,
        coversheetquery.CoversheetID,
        coverstatus_mt.ID as 'CoverStatusID',
        coverstatus_mt.Status as 'CoverStatus',
        covertopic_mt.ID as 'CoverTopicID',
        covertopic_mt.TopicTitle as 'CoverTopicTitle',
        covertype_mt.ID as 'CoverTypeID',
        covertype_mt.Type as 'CoverType',
        coversheetquery.DatePosted,
        coverreplies_tb.RepliedBy,
        coversheetquery.PostedBy,
        coversheetquery.IsReplied,
        coversheetquery.ClosedDate
	FROM coversheetquery
    LEFT JOIN coverstatus_mt ON coverstatus_mt.ID = coversheetquery.coverstatus_mt_ID
    LEFT JOIN covertopic_mt ON covertopic_mt.ID = coversheetquery.covertopic_mt_ID
    LEFT JOIN covertype_mt ON covertype_mt.ID = coversheetquery.covertype_mt_ID
    LEFT JOIN 
    (
		SELECT t1.ID, t1.QueryID, t1.PostedBy as 'RepliedBy'
        FROM coverreplies as t1
        WHERE t1.ID IN 
			(SELECT MAX(t2.ID) as 'MaxID' 
				FROM coverreplies as t2 WHERE t2.QueryID IN (SELECT MQ.ID FROM coversheetquery as MQ WHERE MQ.CoversheetID = p_coversheet_id)
				GROUP BY t2.QueryID)
    ) AS coverreplies_tb ON coverreplies_tb.QueryID = coversheetquery.ID
	WHERE 
		coversheetquery.CoversheetID = p_coversheet_id 
        AND
        CASE 
			WHEN p_filterBy <= 3 AND p_filterBy > 0 THEN coverstatus_mt.ID = p_filterBy
            WHEN p_filterBy = 4 THEN coverreplies_tb.RepliedBy = 'LE'
            WHEN p_filterBy = 5 THEN coverreplies_tb.RepliedBy = 'PE'
            ELSE coversheetquery.CoversheetID = p_coversheet_id
		END
	ORDER BY coversheetquery.ID DESC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllQueryManuscript` (`p_id` INT, `p_filterBy` INT)   BEGIN
	DECLARE p_limit int;
	SELECT COUNT(ID) FROM manuscriptquery WHERE JobID = p_id INTO p_limit;
            
    SELECT 
		manuscriptquery.ID,
        manuscriptquery.JobID,
        querystatus_mt.ID as 'QueryStatusID',
        querystatus_mt.Status as 'QueryStatus',
        querytopic_mt.ID as 'QueryTopicID',
        querytopic_mt.TopicTitle as 'QueryTopicTitle',
        querytype_mt.ID as 'QueryTypeID',
        querytype_mt.Type as 'QueryType',
        manuscriptquery.DatePosted,
        queryreplies_tb.RepliedBy,
        manuscriptquery.PostedBy,
        manuscriptquery.IsReplied,
        manuscriptquery.ClosedDate
	FROM manuscriptquery
    LEFT JOIN querystatus_mt ON querystatus_mt.ID = manuscriptquery.querystatus_mt_ID
    LEFT JOIN querytopic_mt ON querytopic_mt.ID = manuscriptquery.querytopic_mt_ID
    LEFT JOIN querytype_mt ON querytype_mt.ID = manuscriptquery.querytype_mt_ID
    LEFT JOIN 
    (
		SELECT t1.ID, t1.QueryID, t1.PostedBy as 'RepliedBy'
        FROM queryreplies as t1
        WHERE t1.ID IN 
			(SELECT MAX(t2.ID) as 'MaxID' 
				FROM queryreplies as t2 WHERE t2.QueryID IN (SELECT MQ.ID FROM manuscriptquery as MQ WHERE MQ.JobID = p_id)
				GROUP BY t2.QueryID)
    ) AS queryreplies_tb ON queryreplies_tb.QueryID = manuscriptquery.ID
	WHERE 
		manuscriptquery.JobID = p_id 
        AND
        CASE 
			WHEN p_filterBy <= 3 AND p_filterBy > 0 THEN querystatus_mt.ID = p_filterBy
            WHEN p_filterBy = 4 THEN queryreplies_tb.RepliedBy = 'LE'
            WHEN p_filterBy = 5 THEN queryreplies_tb.RepliedBy = 'PE'
            ELSE manuscriptquery.JobID = p_id
		END
	ORDER BY manuscriptquery.ID DESC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllQueryReplies` (`p_query_id` INT)   BEGIN
	SELECT * FROM queryreplies 
    WHERE QueryID = p_query_id 
    ORDER BY ID DESC;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllQuerySTP` (`p_stp_id` INT, `p_filterBy` INT)   BEGIN
	DECLARE p_limit int;
	SELECT COUNT(ID) FROM stpquery WHERE StpID = p_stp_id INTO p_limit;
    
	SELECT 
		stpquery.ID,
        stpquery.StpID,
        stpstatus_mt.ID as 'STPStatusID',
        stpstatus_mt.Status as 'STPStatus',
        stptopic_mt.ID as 'STPTopicID',
        stptopic_mt.TopicTitle as 'STPTopicTitle',
        stptype_mt.ID as 'STPTypeID',
        stptype_mt.Type as 'STPType',
        stpquery.DatePosted,
        stpreplies_tb.RepliedBy,
        stpquery.PostedBy,
        stpquery.IsReplied,
        stpquery.ClosedDate
	FROM stpquery
    LEFT JOIN stpstatus_mt ON stpstatus_mt.ID = stpquery.stpstatus_mt_ID
    LEFT JOIN stptopic_mt ON stptopic_mt.ID = stpquery.stptopic_mt_ID
    LEFT JOIN stptype_mt ON stptype_mt.ID = stpquery.stptype_mt_ID
    LEFT JOIN 
    (
		SELECT t1.ID, t1.QueryID, t1.PostedBy as 'RepliedBy'
        FROM stpreplies as t1
        WHERE t1.ID IN 
			(SELECT MAX(t2.ID) as 'MaxID' 
				FROM stpreplies as t2 WHERE t2.QueryID IN (SELECT MQ.ID FROM stpquery as MQ WHERE MQ.StpID = p_stp_id)
				GROUP BY t2.QueryID)
    ) AS stpreplies_tb ON stpreplies_tb.QueryID = stpquery.ID
	WHERE 
		stpquery.StpID = p_stp_id 
        AND
        CASE 
			WHEN p_filterBy <= 3 AND p_filterBy > 0 THEN stpstatus_mt.ID = p_filterBy
            WHEN p_filterBy = 4 THEN stpreplies_tb.RepliedBy = 'LE'
            WHEN p_filterBy = 5 THEN stpreplies_tb.RepliedBy = 'PE'
            ELSE stpquery.StpID = p_stp_id
		END
	ORDER BY stpquery.ID DESC;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllSTPData` (`p_filterBy` INT, `p_BPSProductID` VARCHAR(128), `p_ServiceNumber` VARCHAR(128), `p_SendToPrintStatus` VARCHAR(128))   BEGIN
	SELECT * FROM stp
    WHERE
		CASE 
			WHEN p_filterBy = 1 THEN BPSProductID = p_BPSProductID
            WHEN p_filterBy = 2 THEN ServiceNumber = p_ServiceNumber
            WHEN p_filterBy = 3 THEN SendToPrintStatus = p_SendToPrintStatus
            ELSE stp.ID > 0
		END;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllSTPDataByUser` (`p_Username` VARCHAR(128))   BEGIN
	DECLARE p_EmployeeID INT;
    
    SELECT ID INTO p_EmployeeID FROM employee WHERE UserName = p_Username;

	SELECT * FROM stp WHERE OwnerUserID = p_EmployeeID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllSTPReplies` (`p_query_id` INT)   BEGIN
	SELECT * FROM stpreplies 
    WHERE QueryID = p_query_id 
    ORDER BY ID DESC;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllTurnAroundTime` ()   BEGIN
	SELECT
		...
		DaysPerTaskApproval as TATOnline,
		DaysPerTaskOnline as TATPDFQA,
		A.BenchMarkDays
	FROM jobtrackaunz_v1.turnaroundtime_mt A
	LEFT JOIN jobtrackaunz_v1.updatetype_mt B ON A.ManusType = B.UpdateType;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllUserByAccess` (`p_UserAccessID` INT)   BEGIN
	SELECT * FROM employee WHERE UserAccessID = p_UserAccessID;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetAllUserPassword` ()   BEGIN
	 ...
	 FROM 
		Employee A
	 LEFT JOIN UserAccess_MT B ON A.UserAccessID = B.ID;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetCoversheetDataById` (`p_id` INT)   BEGIN
	SELECT
		A.*,
		B.UserName AS JobOwner
	FROM coversheetdata A 
    LEFT JOIN employee B ON B.ID = A.JobOwner
	WHERE A.CoversheetID = p_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetJobCoversheetDataByProductAndService` (`p_BPSProductID` VARCHAR(10), `p_ServiceNumber` VARCHAR(100))   BEGIN
	SELECT * FROM jobcoversheetdata
    WHERE BPSProductID = p_BPSProductID AND ServiceNumber = p_ServiceNumber
    LIMIT 1;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetManuscriptDataByProductAndService` (`p_BPSProductID` VARCHAR(10), `p_ServiceNumber` VARCHAR(100))   BEGIN
	SELECT *,
		ManuscriptTier as 'CoversheetTier'
    FROM manuscriptdata
    WHERE BPSProductID = p_BPSProductID AND ServiceNumber = p_ServiceNumber
    LIMIT 1;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetManuscriptDataMaxTurnAroundTime` (`p_BPSProductID` VARCHAR(50), `p_ServiceNumber` VARCHAR(100), `p_manuscriptids` VARCHAR(50))   BEGIN
	SELECT 
		...
		A.DateCreated
	 FROM Manuscriptdata A
	 LEFT JOIN turnaroundtime_mt B ON B.ManusType = A.UpdateType
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetManuscriptDetailsById` (`p_manuscript_id` INT)   BEGIN
	SELECT 
		manuscriptdata.ManuscriptID,
		manuscriptdata.JobNumber,
		manuscriptdata.ManuscriptTier,
		manuscriptdata.BPSProductID,
        manuscriptdata.ServiceNumber,
		manuscriptdata.ManuscriptLegTitle,
		manuscriptdata.PETaskNumber
	FROM jobdata
	INNER JOIN manuscriptdata ON manuscriptdata.JobNumber = jobdata.JobNumber
	WHERE manuscriptdata.ManuscriptID = p_manuscript_id
	ORDER BY
		manuscriptdata.ManuscriptID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetNotificationByUser` (`p_User` VARCHAR(128))   BEGIN
	DECLARE p_employeeID INT;
    DECLARE p_userAccessID INT;

    SELECT ID, UserAccessID INTO p_employeeID, p_userAccessID FROM employee WHERE UserName = p_User;
    
    -- DONT DELETE: this is the fundamental structure of notification
    /* SELECT * FROM jobtrackaunz_v1.notification_posts as A
    INNER JOIN jobtrackaunz_v1.notification_useraccess as B ON B.NotificationQueryTypeID = A.NotificationQueryTypeID
    WHERE B.UserAccessID = p_userAccessID; */
    
    SELECT 
		DISTINCT A.ID,
        A.QueryReplyID,
        A.NotificationQueryTypeID,
        G.FirstName,
        B.UserAccessID,
        G.ID as 'EmployeeID',
        CASE
			WHEN A.NotificationQueryTypeID = 1 THEN C.QueryID
            WHEN A.NotificationQueryTypeID = 2 THEN D.QueryID
            WHEN A.NotificationQueryTypeID = 3 THEN E.QueryID
        END as 'QueryID',        
		CASE
			WHEN A.NotificationQueryTypeID = 1 THEN C.Message
            WHEN A.NotificationQueryTypeID = 2 THEN D.Message
            WHEN A.NotificationQueryTypeID = 3 THEN E.Message
        END as 'Message',
        CASE
			WHEN A.NotificationQueryTypeID = 1 THEN C.PostedBy
            WHEN A.NotificationQueryTypeID = 2 THEN D.PostedBy
            WHEN A.NotificationQueryTypeID = 3 THEN E.PostedBy
        END as 'PostedBy',
        CASE
			WHEN A.NotificationQueryTypeID = 1 THEN 'Manuscript'
            WHEN A.NotificationQueryTypeID = 2 THEN 'Coversheet'
            WHEN A.NotificationQueryTypeID = 3 THEN 'STP'
        END as 'JobType',
        CASE
			WHEN F.ID IS NULL THEN false ELSE true
        END as 'IsViewed',
        CASE
			WHEN A.NotificationQueryTypeID = 1 THEN C.DatePosted
            WHEN A.NotificationQueryTypeID = 2 THEN D.DatePosted
            WHEN A.NotificationQueryTypeID = 3 THEN E.DatePosted
        END as 'DatePosted'
    FROM jobtrackaunz_v1.notification_posts A
    INNER JOIN jobtrackaunz_v1.notification_useraccess B ON B.NotificationQueryTypeID = A.NotificationQueryTypeID
    LEFT JOIN jobtrackaunz_v1.queryreplies C ON C.ID = A.QueryReplyID
    LEFT JOIN jobtrackaunz_v1.coverreplies D ON D.ID = A.QueryReplyID
    LEFT JOIN jobtrackaunz_v1.stpreplies E ON E.ID = A.QueryReplyID
    LEFT JOIN jobtrackaunz_v1.notification_userrecord F ON F.NotificationPostID = A.ID
    LEFT JOIN jobtrackaunz_v1.employee G ON G.UserName = 
		CASE
			WHEN A.NotificationQueryTypeID = 1 THEN C.PostedBy
            WHEN A.NotificationQueryTypeID = 2 THEN D.PostedBy
            WHEN A.NotificationQueryTypeID = 3 THEN E.PostedBy
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetPubschedByProductAndService` (`p_BPSProductID` VARCHAR(10), `p_ServiceNumber` VARCHAR(100))   BEGIN
	SELECT *, 
		LegalEditor as 'Editor',
        ProductChargeCode as 'ChargeCode'
	FROM pubsched_mt
    WHERE BPSProductID = p_BPSProductID AND ServiceNumber = p_ServiceNumber
    LIMIT 1;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetQueryCoversheetById` (`p_id` INT)   BEGIN
	SELECT 
		coversheetquery.ID,
        coversheetquery.CoversheetID,
        coverstatus_mt.ID as 'CoverStatusID',
    ...
    LEFT JOIN covertopic_mt ON covertopic_mt.ID = coversheetquery.covertopic_mt_ID
    LEFT JOIN covertype_mt ON covertype_mt.ID = coversheetquery.covertype_mt_ID
    WHERE  coversheetquery.ID = p_id
    ORDER BY coversheetquery.ID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetQueryManuscriptById` (`p_id` INT)   BEGIN
	SELECT 
		manuscriptquery.ID,
        manuscriptquery.JobID,
        querystatus_mt.ID as 'QueryStatusID',
    ...
    LEFT JOIN querytopic_mt ON querytopic_mt.ID = manuscriptquery.querytopic_mt_ID
    LEFT JOIN querytype_mt ON querytype_mt.ID = manuscriptquery.querytype_mt_ID
    WHERE  manuscriptquery.ID = p_id
    ORDER BY manuscriptquery.ID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetQuerySTPById` (`p_id` INT)   BEGIN
	SELECT 
		stpquery.ID,
        stpquery.StpID,
        stpstatus_mt.ID as 'STPStatusID',
    ...
    LEFT JOIN stptopic_mt ON stptopic_mt.ID = stpquery.stptopic_mt_ID
    LEFT JOIN stptype_mt ON stptype_mt.ID = stpquery.stptype_mt_ID
    WHERE  stpquery.ID = p_id
    ORDER BY stpquery.ID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetReportFilterCoversheet` (`p_filterBy` INT)   BEGIN
	SELECT
		DISTINCT
		CASE
			WHEN p_filterBy = 1 THEN CodingDueDate
            WHEN p_filterBy = 2 THEN OnlineDueDate
            WHEN p_filterBy = 3 THEN OnlineTimeliness
		END as 'FilterValue'
    FROM jobtrackaunz_v1.coversheetdata;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetReportFilterManuscript` (`p_filterBy` INT)   BEGIN
	SELECT
		DISTINCT
		CASE
			WHEN p_filterBy = 1 THEN BPSProductID
            WHEN p_filterBy = 2 THEN ServiceNumber
            WHEN p_filterBy = 3 THEN UpdateType
            WHEN p_filterBy = 4 THEN CopyEditDueDate
		END as 'FilterValue'
    FROM jobtrackaunz_v1.manuscriptdata;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetReportFilterSTP` (`p_filterBy` INT)   BEGIN
	SELECT
		DISTINCT
		CASE
			WHEN p_filterBy = 1 THEN BPSProductID
            WHEN p_filterBy = 2 THEN ServiceNumber
            WHEN p_filterBy = 3 THEN SendToPrintStatus
		END as 'FilterValue'
    FROM jobtrackaunz_v1.stp;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetSTPDataById` (`p_id` INT)   BEGIN
	SELECT * FROM stp WHERE ID = p_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetSTPMaxID` ()   BEGIN
	SELECT Max(ID) as 'ID' FROM stp;
END$$
...
-- SQLINES DEMO *** OR EVALUATION USE ONLY
-- SQLINES LICENSE FOR EVALUATION USE ONLY
Select ID from employee where UserName = p_varchar_userName;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `GetUserNameByUserID` (`p_int_userID` INT)   BEGIN
-- SQLINES DEMO *** OR EVALUATION USE ONLY
-- SQLINES LICENSE FOR EVALUATION USE ONLY
Select UserName from employee where ID = p_int_userID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertCoversheet` (`p_Username` VARCHAR(45), `p_CoversheetNumber` VARCHAR(200), `p_BPSProductID` VARCHAR(50), `p_ServiceNumber` VARCHAR(100), `p_TaskNumber` VARCHAR(100), `p_CoversheetTier` VARCHAR(50), `p_Editor` VARCHAR(100), `p_ChargeCode` VARCHAR(100), `p_TargetPressDate` DATETIME, `p_TaskType` VARCHAR(50), `p_GuideCard` VARCHAR(500), `p_LocationOfManuscript` VARCHAR(500), `p_UpdateType` VARCHAR(50), `p_GeneralLegRefCheck` VARCHAR(45), `p_GeneralTOC` VARCHAR(45), `p_GeneralTOS` VARCHAR(45), `p_GeneralReprints` VARCHAR(45), `p_GeneralFascicleInsertion` VARCHAR(45), `p_GeneralGraphicLink` VARCHAR(45), `p_GeneralGraphicEmbed` VARCHAR(45), `p_GeneralHandtooling` VARCHAR(45), `p_GeneralNonContent` VARCHAR(45), `p_GeneralSamplePages` VARCHAR(45), `p_GeneralComplexTask` VARCHAR(45), `p_FurtherInstruction` VARCHAR(60000), `p_CodingDueDate` DATETIME, `p_IsXMLEditing` VARCHAR(50), `p_OnlineDueDate` DATETIME, `p_IsOnline` VARCHAR(50), `p_ManuscriptID` VARCHAR(45))   BEGIN
	DECLARE p_EmployeeID INT;
    ...
    -- get LatestTaskNumber id
    SELECT LatestTaskNumber INTO p_LatestTaskNumber FROM jobcoversheetdata WHERE BPSProductID = p_BPSProductID AND ServiceNumber = p_ServiceNumber;
    
    -- update latesttasknumber of jobcoversheet
    UPDATE jobcoversheetdata SET LatestTaskNumber = p_LatestTaskNumber + 1 WHERE BPSProductID = p_BPSProductID AND ServiceNumber = p_ServiceNumber;
	...
	THEN
		...
		VALUES
            ...
            , p_TaskType
            , p_GuideCard
            , p_LocationOfManuscript
            , p_UpdateType
            , p_GeneralLegRefCheck
            , p_GeneralTOC
            , p_GeneralTOS
            , p_GeneralReprints
            , p_GeneralFascicleInsertion
            , p_GeneralGraphicLink
            , p_GeneralGraphicEmbed
            , p_GeneralHandtooling
            , p_GeneralNonContent
            , p_GeneralSamplePages
            , p_GeneralComplexTask
            , p_FurtherInstruction
            , p_CodingDueDate
            , p_IsXMLEditing
            , p_OnlineDueDate
            , p_IsOnline
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertJob` (`p_Username` VARCHAR(45), `p_ManuscriptTier` VARCHAR(50), `p_BPSProductID` VARCHAR(50), `p_ServiceNumber` VARCHAR(100), `p_ManuscriptLegTitle` VARCHAR(1000), `p_TargetPressDate` DATE, `p_LatupAttribution` VARCHAR(1000), `p_DateReceivedFromAuthor` DATE, `p_UpdateType` VARCHAR(50), `p_JobSpecificInstruction` VARCHAR(500), `p_TaskType` VARCHAR(50), `p_CopyEditDueDate` DATE, `p_CodingDueDate` DATE, `p_OnlineDueDate` DATE)   BEGIN
	...
	FROM turnaroundtime_mt A
	LEFT JOIN updatetype_mt B
	...
	IF p_EmployeeID IS NOT NULL
	THEN
		INSERT INTO JobData(ManuscriptTier
			, BPSProductID
            , ServiceNumber
            , TargetPressDate
            , CopyEditStatus
		...
		VALUES
            ...
            , p_BPSProductID
            , p_ServiceNumber
            , p_ManuscriptLegTitle
            , p_TargetPressDate
            , p_LatupAttribution
            , p_DateReceivedFromAuthor
            , p_RealUpdateType
            , p_JobSpecificInstruction
            , p_TaskType
            , p_CopyEditDueDate
            , p_CodingDueDate
            , p_OnlineDueDate
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertJobCoversheet` (`p_Username` VARCHAR(45), `p_CoversheetNumber` VARCHAR(200), `p_BPSProductID` VARCHAR(50), `p_ServiceNumber` VARCHAR(100), `p_TaskNumber` VARCHAR(100), `p_CoversheetTier` VARCHAR(50), `p_Editor` VARCHAR(100), `p_ChargeCode` VARCHAR(100), `p_TargetPressDate` DATETIME, `p_TaskType` VARCHAR(50), `p_GuideCard` VARCHAR(500), `p_LocationOfManuscript` VARCHAR(500), `p_UpdateType` VARCHAR(50), `p_GeneralLegRefCheck` VARCHAR(45), `p_GeneralTOC` VARCHAR(45), `p_GeneralTOS` VARCHAR(45), `p_GeneralReprints` VARCHAR(45), `p_GeneralFascicleInsertion` VARCHAR(45), `p_GeneralGraphicLink` VARCHAR(45), `p_GeneralGraphicEmbed` VARCHAR(45), `p_GeneralHandtooling` VARCHAR(45), `p_GeneralNonContent` VARCHAR(45), `p_GeneralSamplePages` VARCHAR(45), `p_GeneralComplexTask` VARCHAR(45), `p_FurtherInstruction` VARCHAR(6000), `p_CodingDueDate` DATETIME, `p_IsXMLEditing` BIT, `p_OnlineDueDate` DATETIME, `p_IsOnline` BIT, `p_ManuscriptID` VARCHAR(45))   BEGIN
	DECLARE p_EmployeeID INT;
    ...
    IF p_EmployeeID IS NOT NULL
    THEN
		INSERT INTO JobCoversheetData(CoversheetTier
			, BPSProductID
            , ServiceNumber
            , TargetPressDate
            , CodingStatus
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertJobTransactionLog` (`p_BPSProductID` VARCHAR(50), `p_ServiceNumber` VARCHAR(50), `p_UserName` VARCHAR(45))   BEGIN
	DECLARE is_Exist INT;
	DECLARE p_PEUserName varchar(45);
	DECLARE p_LEUserName varchar(45);    
	DECLARE p_JobID int;
    
    -- count transaction_type based on product_id and service_number for jobreassignment
    SET is_Exist = (SELECT COUNT(*) TransactionType FROM TransactionLog
		WHERE BPSProductID = p_BPSProductID
		AND ServiceNumber = p_ServiceNumber
		AND	TransactionType LIKE 'JobReassignment%');
        
	IF is_Exist = 0
    THEN
		-- select PE and LE usernames
		SELECT PEUserName, LEUserName INTO p_PEUsername, p_LEUserName
		FROM PublicationAssignment WHERE BPSProductID = p_BPSProductID
		LIMIT 1;
        
        -- get the last_id from jobdata
        SELECT MAX(JobID) INTO p_JobID FROM JobData
        WHERE BPSProductID = p_BPSProductID
			AND ServiceNumber = p_ServiceNumber;
            
		IF p_LEUserName IS NOT NULL
		THEN
			-- if LE username is not null, assign it as ValueAfter
			INSERT INTO TransactionLog(TransactionLogIdentity
				, TransactionLogName
                , BPSProductID
                , ServiceNumber
                , TransactionType
		...
		THEN
			-- if PE username is not null, assign it as ValueAfter
			INSERT INTO TransactionLog(TransactionLogIdentity
				, TransactionLogName
                , BPSProductID
                , ServiceNumber
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertManuscript` (`p_Username` VARCHAR(45), `p_ManuscriptTier` VARCHAR(50), `p_BPSProductID` VARCHAR(50), `p_ServiceNumber` VARCHAR(100), `p_ManuscriptLegTitle` VARCHAR(1000), `p_TargetPressDate` DATE, `p_LatupAttribution` VARCHAR(1000), `p_DateReceivedFromAuthor` DATE, `p_UpdateType` VARCHAR(50), `p_JobSpecificInstruction` VARCHAR(500), `p_TaskType` VARCHAR(50), `p_CopyEditDueDate` DATE, `p_CodingDueDate` DATE, `p_OnlineDueDate` DATE)   BEGIN
	...
	SELECT A.ManusType as UpdateType INTO p_RealUpdateType
	FROM turnaroundtime_mt A
	LEFT JOIN updatetype_mt B ON A.ManusType = B.UpdateType
	WHERE TurnAroundTimeID = p_UpdateType;

	-- SELECT COALESCE(MAX(ManuscriptID), 0) + 1 as ManuscriptID INTO p_JobNumber
	-- from jobtrackaunz.manuscriptdata;

	-- get job_number based on product_id and service_number
	SELECT JobNumber INTO p_JobNumber FROM jobdata
	WHERE BPSProductID = p_BPSProductID
	AND ServiceNumber = p_ServiceNumber;

	-- insert manuscript_data with job_number(JobNumber), manus_type(UpdateType) & employee_id(CreatedEmployeeID/UpdateEmployeeID)
	IF p_EmployeeID IS NOT NULL
	THEN
		INSERT INTO manuscriptdata(JobNumber
			, ManuscriptTier
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertSendToPrint` (`p_SendToPrintNumber` VARCHAR(100), `p_BPSProductID` VARCHAR(100), `p_ServiceNumber` VARCHAR(100), `p_SendToPrintTier` VARCHAR(50), `p_TargetPressDate` DATETIME, `p_PathOfInputFiles` VARCHAR(2000), `p_SpecialInstruction` VARCHAR(2000), `p_IsConsoHighlight` SMALLINT(6), `p_IsFilingInstruction` SMALLINT(6), `p_IsDummyFiling1` SMALLINT(6), `p_IsDummyFiling2` SMALLINT(6), `p_IsUECJ` SMALLINT(6), `p_IsPC1PC2` SMALLINT(6), `p_IsReadyToPrint` SMALLINT(6), `p_IsSendingFinalPages` SMALLINT(6), `p_IsPostBack` SMALLINT(6), `p_IsUpdateEBinder` SMALLINT(6), `p_CoversheetIDs` VARCHAR(50), `p_Username` VARCHAR(50))   BEGIN
	DECLARE p_EmployeeID INT;
    DECLARE p_NewSTPID INT;
    ...
    IF p_EmployeeID IS NOT NULL
	THEN
		INSERT INTO stp(SendToPrintNumber
			, BPSProductID
            , ServiceNumber
            , SendToPrintTier
            , TargetPressDate
            -- , LegislationMaterials
            , PathOfInputFiles
            , SpecialInstruction
            , IsConsoHighlight
            , IsFilingInstruction
		...
		VALUES
            ...
            , p_SendToPrintTier
            , p_TargetPressDate
            -- , p_LegislationMaterials
            , p_PathOfInputFiles
            , p_SpecialInstruction
            , p_IsConsoHighlight
            , p_IsFilingInstruction
            , p_IsDummyFiling1
            , p_IsDummyFiling2
            , p_IsUECJ
            , p_IsPC1PC2
            , p_IsReadyToPrint
            , P_IsSendingFinalPages
            , p_IsPostBack
            , p_IsUpdateEBinder
            , "New"
            , utc_timestamp()
		...
		-- get new id
        SELECT last_insert_id() INTO p_NewSTPID;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateCoversheetData` (`p_CoversheetID` INT, `p_Username` VARCHAR(128), `p_UserAccess` INT, `p_LocationOfManuscript` VARCHAR(250), `p_FurtherInstruction` VARCHAR(250), `p_GuideCard` VARCHAR(250), `p_GeneralLegRefCheck` BIT, `p_GeneralTOC` BIT, `p_GeneralTOS` BIT, `p_GeneralReprints` BIT, `p_GeneralFascicleInsertion` BIT, `p_GeneralGraphicLink` BIT, `p_GeneralGraphicEmbed` BIT, `p_GeneralHandtooling` BIT, `p_GeneralNonContent` BIT, `p_GeneralSamplePages` BIT, `p_GeneralComplexTask` BIT, `p_RevisedOnlineDueDate` DATETIME, `p_Remarks` VARCHAR(250), `p_IsXMLEditing` BIT, `p_IsOnline` BIT, `p_JobOwner` VARCHAR(128), `p_JobOwnerID` INT, `p_CompletionEmailTemplate` VARCHAR(1000), `p_CodingStartDate` DATETIME, `p_CodingDoneDate` DATETIME, `p_PDFQCStartDate` DATETIME, `p_PDFQCDoneDate` DATETIME, `p_OnlineStartDate` DATETIME, `p_OnlineDoneDate` DATETIME, `p_AttachmentFile` VARCHAR(1000))   BEGIN
	DECLARE p_EmployeeID INT;

	-- get employee ID
	SELECT ID INTO p_EmployeeID FROM employee WHERE Username = p_Username;

	IF p_UserAccess = 3		-- PE
    THEN
    ...
    THEN
		...
		SET
			...
			IsOnline = p_IsOnline,
            JobOwner = p_JobOwner,
            JobOwnerID = p_JobOwnerID,
            UpdateEmployeeID = p_EmployeeID
		WHERE CoversheetID = p_CoversheetID;
        
	ELSEIF p_UserAccess = 5		-- Coding
    THEN
		UPDATE coversheetdata
		SET
			-- GeneralLegRefCheck = p_GeneralLegRefCheck,
			...
			IsXMLEditing = p_IsXMLEditing,
			IsOnline = p_IsOnline,
            CompletionEmailTemplate = p_CompletionEmailTemplate,
            ...
            AttachmentFile = p_AttachmentFile,
            UpdateEmployeeID = p_EmployeeID
		WHERE CoversheetID = p_CoversheetID;
    END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateEmployee` (`p_ID` INT(11), `p_UserAccessID` INT(11), `p_UserName` VARCHAR(45), `p_Password` VARCHAR(200), `p_FirstName` VARCHAR(150), `p_LastName` VARCHAR(150), `p_EmailAddress` VARCHAR(100), `p_IsManager` SMALLINT(6), `p_IsEditorialContact` SMALLINT(6), `p_IsEmailList` SMALLINT(6), `p_IsMandatoryRecepient` SMALLINT(6), `p_IsShowUser` SMALLINT(6), `p_User` VARCHAR(150))   BEGIN
	DECLARE p_is_update BIT;

	IF EXISTS (SELECT * FROM employee WHERE ID = p_ID)
    THEN
		UPDATE employee
        SET UserAccessID = p_UserAccessID
			, UserName = p_UserName
            , Password = p_Password
            ...
            , IsEditorialContact = p_IsEditorialContact
            , IsEmailList = p_IsEmailList
            , IsMandatoryRecepient = p_IsMandatoryRecepient
            , IsShowUser = p_IsShowUser
            , PasswordUpdateDate = utc_timestamp()
		WHERE
			ID = p_ID;
            
		SELECT 1 INTO p_is_update;
    ELSE
		INSERT INTO employee(UserAccessID
			, UserName
            , Password
            , FirstName
            , LastName
        ...
        SELECT p_UserAccessID
            ...
            , p_EmailAddress
            , p_IsManager
            , p_IsEditorialContact
            , p_IsEmailList
            , p_IsMandatoryRecepient
            , p_IsShowUser
            , utc_timestamp()
            , CONCAT(p_LastName, ', ', p_FirstName);
    ...
    THEN		
        CALL UpdateHistoryTrail(p_User
			, p_ID
			, CONCAT('Employee [', p_UserName, '] details has been updated by [', p_user, '].') 
            , 'Updated'
            , null);
	ELSE		
        CALL UpdateHistoryTrail(p_User
			, p_ID
			, CONCAT('New Employee [', p_UserName, ']  has been added by [', p_user, '].') 
            , "New"
            , null);
    END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateJobData` (`p_id` INT, `p_TargetPressDate` DATETIME, `p_ActualPressDate` DATETIME, `p_CopyEditStatus` NVARCHAR(500), `p_CodingStatus` NVARCHAR(500), `p_OnlineStatus` NVARCHAR(500), `p_STPStatus` NVARCHAR(500), `p_Username` NVARCHAR(128))   BEGIN
    ...
    SELECT ID INTO p_employee_id FROM employee WHERE UserName = p_Username;

	UPDATE jobdata
    SET TargetPressDate = p_TargetPressDate
		, ActualPressDate = p_ActualPressDate
        , CopyEditStatus = p_CopyEditStatus
        , CodingStatus = p_CodingStatus
        , OnlineStatus = p_OnlineStatus
        , STPStatus = p_STPStatus
        , UpdateEmployeeID = p_employee_id
...
IF p_EmployeeID IS NOT NULL
THEN
INSERT INTO Manuscript(JobNumber, ManuscriptTier, BPSProductID, ServiceNumber, ManuscriptLegTitle, TargetPressDate, ActualPressDate,
			LatupAttribution, DateReceivedFromAuthor, UpdateType, JobSpecificInstruction, 
			TaskType, CopyEditDueDate, CodingDueDate, OnlineDueDate, STPStatus, DateCreated, CreatedEmployeeID, DateUpdated, UpdateEmployeeID)
VALUES( 
p_JobNumber, p_ManuscriptTier, p_BPSProductID, p_ServiceNumber, p_ManuscriptLegTitle, p_TargetPressDate, 19000101,
p_LatupAttribution, p_DateReceivedFromAuthor, p_RealUpdateType, p_JobSpecificInstruction,
p_TaskType, p_CopyEditDueDate, p_CodingDueDate, p_OnlineDueDate, p_STPStatus, CURRENT_TIMESTAMP, p_EmployeeID, CURRENT_TIMESTAMP, p_EmployeeID
);
END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateManuscriptData` (`p_UserName` VARCHAR(128), `p_ManuscriptID` INT, `p_Remarks` VARCHAR(1000), `p_GuideCard` VARCHAR(500), `p_RevisedOnlineDueDate` DATETIME, `p_CopyEditStartDate` DATETIME, `p_CopyEditDoneDate` DATETIME, `p_CopyEditStatus` VARCHAR(128), `p_CodingStartDate` DATETIME, `p_CodingDoneDate` DATETIME, `p_CodingStatus` VARCHAR(128), `p_OnlineStartDate` DATETIME, `p_OnlineDoneDate` DATETIME, `p_OnlineStatus` VARCHAR(128), `p_LatupAttribution` NVARCHAR(250), `p_EstimatedPages` INT, `p_UpdateType` NVARCHAR(128))   BEGIN
	DECLARE p_EmployeeID INT;
    ...
    SET
        ...
        , UpdateEmployeeID = p_EmployeeID
	WHERE
		ManuscriptID = p_ManuscriptID;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateNotificationPost` (`p_ReplyID` INT, `p_NotificationPostID` INT, `p_CreatedBy` INT)   BEGIN
	INSERT INTO notification_posts(
		QueryReplyID
        , NotificationQueryTypeID
        , DateCreated
        , CreatedBy
    )
    SELECT
		p_ReplyID
        , p_NotificationPostID
        , utc_timestamp()
        , p_CreatedBy;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateNotificationUserRecord` (`p_NotificationPostID` INT, `p_EmployeeID` INT)   BEGIN
	IF NOT EXISTS (SELECT * FROM notification_userrecord WHERE NotificationPostID = p_NotificationPostID AND EmployeeID = p_EmployeeID)
    THEN
		INSERT INTO notification_userrecord(NotificationPostID, EmployeeID)
        SELECT p_NotificationPostID, p_EmployeeID;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdatePublicationAssignment` (`p_id` INT, `p_BPSProductID` VARCHAR(50), `p_CompleteNameOfPublication` VARCHAR(500), `p_PublicationTier` VARCHAR(50), `p_PEName` VARCHAR(200), `p_PEEmail` VARCHAR(200), `p_PEUserName` VARCHAR(45), `p_PEStatus` VARCHAR(50), `p_LEName` VARCHAR(200), `p_LEEmail` VARCHAR(200), `p_LEUserName` VARCHAR(45), `p_LEStatus` VARCHAR(50), `p_CreatedEmployeeID` INT, `p_UpdateEmployeeID` INT, `p_User` VARCHAR(150))   BEGIN
	DECLARE p_ID INT;

    SELECT ID INTO p_CreatedEmployeeID FROM employee WHERE UserName = p_User;

	INSERT INTO publicationassignment(
		BPSProductID
        , CompleteNameOfPublication
        , PublicationTier
        , PEName
        , PEEmail
        , PEUserName
    ...
    CALL UpdateHistoryTrail(p_User
			, p_ID
			, CONCAT('New Product ID [', p_BPSProductID, ']  has been created by [', p_User, '].') 
            , "New"
            , null);
        
	-- return new id
	SELECT p_ID as 'PublicationAssignmentID';
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateQueryCoversheet` (`p_id` INT, `p_coversheet_id` INT, `p_coverstatus_id` INT, `p_covertopic_id` INT, `p_covertype_id` INT, `p_filepath` NVARCHAR(1000), `p_user` NVARCHAR(128))   BEGIN
	DECLARE p_is_update BIT;
    DECLARE p_status VARCHAR(50);
    
    SELECT Status INTO p_status FROM jobtrackaunz_v1.coverstatus_mt WHERE ID = p_coverstatus_id;

	IF EXISTS (SELECT * FROM coversheetquery WHERE ID = p_id)
    THEN
		UPDATE coversheetquery
        SET	coverstatus_mt_ID = p_coverstatus_id,
			covertopic_mt_ID = p_covertopic_id,
            covertype_mt_ID = p_covertype_id,
            FilePath = p_filepath
		WHERE
			ID = p_id;
        ...
        SELECT 1 INTO p_is_update;
    ELSE
		INSERT INTO coversheetquery(CoversheetID
            , coverstatus_mt_ID
            , covertopic_mt_ID
            , covertype_mt_ID
            , FilePath
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateQueryCoversheetStatus` (`p_id` INT, `p_coversheet_id` INT, `p_coverstatus_id` INT, `p_repliedby` VARCHAR(128), `p_is_statuschanged` BIT, `p_close_date` DATETIME)   BEGIN
	DECLARE p_status VARCHAR(50);
    
    SELECT Status INTO p_status FROM jobtrackaunz_v1.coverstatus_mt WHERE ID = p_coverstatus_id;

	IF p_coverstatus_id = 3
    THEN
		UPDATE coversheetquery
		SET coverstatus_mt_ID = p_coverstatus_id,
			IsReplied = 1,
            ClosedDate = p_close_date
		WHERE ID = p_id;
    ELSE
		UPDATE coversheetquery
		SET coverstatus_mt_ID = p_coverstatus_id,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateQueryManuscript` (`p_id` INT, `p_job_id` INT, `p_querystatus_id` INT, `p_querytopic_id` INT, `p_querytype_id` INT, `p_filepath` NVARCHAR(1000), `p_user` NVARCHAR(128))   BEGIN
	...
	IF EXISTS (SELECT * FROM manuscriptquery WHERE ID = p_id)
    THEN
		UPDATE manuscriptquery
        SET	querystatus_mt_ID = p_querystatus_id,
			querytopic_mt_ID = p_querytopic_id,
            querytype_mt_ID = p_querytype_id,
            FilePath = p_filepath
		WHERE
			ID = p_id;
		...
		SELECT 1 INTO p_is_update;
    ELSE
		INSERT INTO manuscriptquery(JobID
            , querystatus_mt_ID
            , querytopic_mt_ID
            , querytype_mt_ID
            , FilePath
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateQueryManuscriptStatus` (`p_id` INT, `p_job_id` INT, `p_querystatus_id` INT, `p_repliedby` VARCHAR(128), `p_is_statuschanged` BIT, `p_close_date` DATETIME)   BEGIN
	DECLARE p_status VARCHAR(50);
    
    SELECT Status INTO p_status FROM jobtrackaunz_v1.querystatus_mt WHERE ID = p_querystatus_id;
    
	IF p_querystatus_id = 3
    THEN
		UPDATE manuscriptquery
		SET querystatus_mt_ID = p_querystatus_id,
			IsReplied = 1,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateQuerySTP` (`p_id` INT, `p_stp_id` INT, `p_stpstatus_id` INT, `p_stptopic_id` INT, `p_stptype_id` INT, `p_filepath` NVARCHAR(1000), `p_user` NVARCHAR(128))   BEGIN
	...
	IF EXISTS (SELECT * FROM stpquery WHERE ID = p_id)
    THEN
		UPDATE stpquery
        SET	stpstatus_mt_ID = p_stpstatus_id,
			stptopic_mt_ID = p_stptopic_id,
            stptype_mt_ID = p_stptype_id,
            FilePath = p_filepath
		WHERE
			ID = p_id;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateQuerySTPStatus` (`p_id` INT, `p_stp_id` INT, `p_stpstatus_id` INT, `p_repliedby` VARCHAR(128), `p_is_statuschanged` BIT, `p_close_date` DATETIME)   BEGIN
	DECLARE p_status VARCHAR(50);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateReply` (`p_id` INT, `p_query_id` INT, `p_message` NVARCHAR(250))   BEGIN
	IF EXISTS (SELECT * FROM queryreplies WHERE ID = p_id)
    THEN
		UPDATE queryreplies
		...
		WHERE
			ID = p_id;
    ELSE
		INSERT INTO queryreplies(QueryID
            , Message
            , DatePosted)
		SELECT
			p_query_id,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateReplyCoversheet` (`p_id` INT, `p_query_id` INT, `p_message` NVARCHAR(250), `p_user` NVARCHAR(128), `p_dateposted` DATETIME)   BEGIN
	DECLARE p_isNew BIT;
	DECLARE p_employeeID INT;    
    SELECT ID INTO p_employeeID FROM employee WHERE UserName = p_user;
    
	IF EXISTS (SELECT * FROM coverreplies WHERE ID = p_id)
    THEN
		UPDATE coverreplies
        SET	Message = p_message,
			DatePosted = p_dateposted,
            PostedBy = p_user
		WHERE ID = p_id;
    ELSE
		INSERT INTO coverreplies(
			QueryID
            , Message
            , DatePosted
            , PostedBy)
		...
		-- get new id
        SELECT last_insert_id() INTO p_id;
        SELECT true INTO p_isNew;
    END IF;
    
    -- update notification
    CALL UpdateNotificationPost(
		p_id
		, 2 -- coversheet
        , p_employeeID);
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateReplyManuscript` (`p_id` INT, `p_query_id` INT, `p_message` NVARCHAR(250), `p_user` NVARCHAR(128), `p_dateposted` DATETIME)   BEGIN
	DECLARE p_isNew BIT;
	DECLARE p_employeeID INT;    
    SELECT ID INTO p_employeeID FROM employee WHERE UserName = p_user;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateReplySTP` (`p_id` INT, `p_query_id` INT, `p_message` NVARCHAR(250), `p_user` NVARCHAR(128), `p_dateposted` DATETIME)   BEGIN
	DECLARE p_isNew BIT;
	DECLARE p_employeeID INT;    
    SELECT ID INTO p_employeeID FROM employee WHERE UserName = p_user;
    
	IF EXISTS (SELECT * FROM stpreplies WHERE ID = p_id)
    THEN
		UPDATE stpreplies
        SET	Message = p_message,
			DatePosted = p_dateposted,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateSendToPrint` (`p_ID` INT, `p_JobOwnerID` INT, `p_SendToPrintNumber` VARCHAR(100), `p_BPSProductID` VARCHAR(100), `p_ServiceNumber` VARCHAR(100), `p_SendToPrintTier` VARCHAR(50), `p_TargetPressDate` DATETIME, `p_PathOfInputFiles` VARCHAR(2000), `p_SpecialInstruction` VARCHAR(2000), `p_IsConsoHighlight` SMALLINT(6), `p_IsFilingInstruction` SMALLINT(6), `p_IsDummyFiling1` SMALLINT(6), `p_IsDummyFiling2` SMALLINT(6), `p_IsUECJ` SMALLINT(6), `p_IsPC1PC2` SMALLINT(6), `p_IsReadyToPrint` SMALLINT(6), `p_IsSendingFinalPages` SMALLINT(6), `p_IsPostBack` SMALLINT(6), `p_IsUpdateEBinder` SMALLINT(6), `p_CoversheetIDs` VARCHAR(50), `p_SendToPrintStatus` VARCHAR(50), `p_Username` VARCHAR(50), `p_ConsoleHighlightStartDate` DATETIME, `p_ConsoleHighlightEndDate` DATETIME, `p_FilingInstructionStartDate` DATETIME, `p_FilingInstructionEndDate` DATETIME, `p_DummyFiling1StartDate` DATETIME, `p_DummyFiling1EndDate` DATETIME, `p_DummyFiling2StartDate` DATETIME, `p_DummyFiling2EndDate` DATETIME, `p_UECJStartDate` DATETIME, `p_UECJEndDate` DATETIME, `p_PC1PC2StartDate` DATETIME, `p_PC1PC2EndDate` DATETIME, `p_PostBackStartDate` DATETIME, `p_PostBackEndDate` DATETIME, `p_UpdateEBinderStartDate` DATETIME, `p_UpdateEBinderEndDate` DATETIME)   BEGIN
	DECLARE p_EmployeeID INT;
    DECLARE p_AssignedUser VARCHAR(128);
      
	-- get employee id
	SELECT ID INTO p_EmployeeID FROM employee WHERE Username = p_Username;
    
    -- get job owner username
    SELECT UserName INTO p_AssignedUser FROM employee WHERE ID = p_JobOwnerID;
    
    UPDATE stp
    SET 
        OwnerUserID = p_JobOwnerID
		, PathOfInputFiles = p_PathOfInputFiles
		, SpecialInstruction = p_SpecialInstruction
		, IsConsoHighlight = p_IsConsoHighlight
		, IsFilingInstruction = p_IsFilingInstruction
		, IsDummyFiling1 = p_IsDummyFiling1
		, IsDummyFiling2 = p_IsDummyFiling2
		, IsUECJ = p_IsUECJ
		, IsPC1PC2 = p_IsPC1PC2
		, IsReadyToPrint = p_IsReadyToPrint
		, IsSendingFinalPages = IsSendingFinalPages
		, IsPostBack = p_IsPostBack
		, IsUpdateEBinder = p_IsUpdateEBinder
		, SendToPrintStatus = p_SendToPrintStatus		
		, DateUpdated = utc_timestamp()
		, UpdateEmployeeID = p_EmployeeID
        
        , ConsoleHighlightStartDate = p_ConsoleHighlightStartDate
        , ConsoleHighlightEndDate = p_ConsoleHighlightEndDate 
        , FilingInstructionStartDate = p_FilingInstructionStartDate 
        , FilingInstructionEndDate = p_FilingInstructionEndDate 
        , DummyFiling1StartDate = p_DummyFiling1StartDate 
        , DummyFiling1EndDate = p_DummyFiling1EndDate 
        , DummyFiling2StartDate = p_DummyFiling2StartDate 
        , DummyFiling2EndDate = p_DummyFiling2EndDate 
        , UECJStartDate = p_UECJStartDate 
        , UECJEndDate = p_UECJEndDate 
        , PC1PC2StartDate = p_PC1PC2StartDate 
        , PC1PC2EndDate = p_PC1PC2EndDate 
        , PostBackStartDate = p_PostBackStartDate 
        , PostBackEndDate = p_PostBackEndDate 
        , UpdateEBinderStartDate = p_UpdateEBinderStartDate 
        , UpdateEBinderEndDate = p_UpdateEBinderEndDate 
		-- SendToPrintNumber = p_SendToPrintNumber
		-- , BPSProductID = p_BPSProductID
		-- , ServiceNumber = p_ServiceNumber
		-- , SendToPrintTier = p_SendToPrintTier
		-- , TargetPressDate = p_TargetPressDate
		-- , LegislationMaterials
        -- , DateCreated = p_DateCreated
		-- , CreatedEmployeeID = p_CreatedEmployeeID
        -- , CoversheetIDs = p_CoversheetIDs
	WHERE
		ID = p_ID;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateSubsequentPass` (`p_UserName` VARCHAR(128), `p_CoversheetID` INT, `p_SubsequentPass` VARCHAR(5000))   BEGIN
	...
	-- reset coversheet data and update subsequent
	UPDATE coversheetdata
    SET
		SubsequentPass = p_SubsequentPass,
        CodingStartDate = null,
        CodingDoneDate = null,
        PDFQCStartDate = null,
        PDFQCDoneDate = null,
        UpdateEmployeeID = p_EmployeeID
	WHERE
		CoversheetID = p_CoversheetID;
END$$
...
--
-- Table structure for table `coverreplies`
--

CREATE TABLE `coverreplies` (
  `ID` int(11) NOT NULL,
...
CREATE TABLE `coversheetdata` (
  ...
  `GeneralLegRefCheck` bit(1) DEFAULT NULL,
  `GeneralTOC` bit(1) DEFAULT NULL,
  `GeneralTOS` bit(1) DEFAULT NULL,
  `GeneralReprints` bit(1) DEFAULT NULL,
  `GeneralFascicleInsertion` bit(1) DEFAULT NULL,
  `GeneralGraphicLink` bit(1) DEFAULT NULL,
  `GeneralGraphicEmbed` bit(1) DEFAULT NULL,
  `GeneralHandtooling` bit(1) DEFAULT NULL,
  `GeneralNonContent` bit(1) DEFAULT NULL,
  `GeneralSamplePages` bit(1) DEFAULT NULL,
  `GeneralComplexTask` bit(1) DEFAULT NULL,
  `FurtherInstruction` varchar(6000) DEFAULT NULL,
  `UpdateType` varchar(50) DEFAULT NULL,
  `AcceptedDate` datetime DEFAULT NULL,
  `JobOwner` varchar(128) DEFAULT NULL,
  `JobOwnerID` int(11) DEFAULT NULL,
  `UpdateEmailCC` varchar(1000) DEFAULT NULL,
  `IsXMLEditing` bit(1) DEFAULT NULL,
  ...
  `CompletionEmailTemplate` varchar(1000) DEFAULT NULL,
  `SubsequentPass` varchar(5000) DEFAULT NULL,
  `AttachmentFile` varchar(1000) DEFAULT NULL,
...
CREATE TABLE `coversheetquery` (
  `ID` int(11) NOT NULL,
  `CoversheetNo` varchar(200) DEFAULT NULL,
  `QueryID` int(11) DEFAULT NULL,
  `CoversheetID` int(11) DEFAULT NULL,
  `covertopic_mt_ID` int(11) DEFAULT NULL,
  `coverstatus_mt_ID` int(11) DEFAULT NULL,
  `covertype_mt_ID` int(11) DEFAULT NULL,
  `FilePath` varchar(1000) DEFAULT NULL,
  `IsReplied` bit(1) NOT NULL DEFAULT b'0',
  `DatePosted` datetime DEFAULT NULL,
...
--
-- Table structure for table `coverstatus_mt`
--

CREATE TABLE `coverstatus_mt` (
  `ID` int(11) NOT NULL,
  `Status` varchar(128) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;

--
-- Dumping data for table `coverstatus_mt`
--

INSERT INTO `coverstatus_mt` (`ID`, `Status`) VALUES
(1, 'Open'),
(2, 'Cancelled'),
(3, 'Closed');
...
--
-- Table structure for table `covertopic_mt`
--

CREATE TABLE `covertopic_mt` (
  `ID` int(11) NOT NULL,
  `TopicTitle` varchar(128) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;

--
-- Dumping data for table `covertopic_mt`
--

INSERT INTO `covertopic_mt` (`ID`, `TopicTitle`) VALUES
(1, 'Topic 1'),
(2, 'Topic 2'),
(3, 'Topic 3');
...
--
-- Table structure for table `covertype_mt`
--

CREATE TABLE `covertype_mt` (
  `ID` int(11) NOT NULL,
  `Type` varchar(128) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;

--
-- Dumping data for table `covertype_mt`
--

INSERT INTO `covertype_mt` (`ID`, `Type`) VALUES
(1, 'General'),
(2, 'XML Tagging'),
(3, 'Format');
...
INSERT INTO `employee` (`ID`, `UserAccessID`, `CreatedDate`, `UserName`, `Password`, `FirstName`, `LastName`, `EmailAddress`, `IsManager`, `IsEditorialContact`, `IsEmailList`, `IsMandatoryRecepient`, `IsShowUser`, `PasswordUpdateDate`, `FullName`) VALUES
...
(8, 2, '2023-03-24 23:44:01', 'Jennifer.Murray', 'Jennifer.Murray', 'Jennifer', 'Murray', 'Jennifer.Murray@example.com', 0, 1, 1, 1, 1, '0000-00-00 00:00:00', 'Murray, Jennifer'),
(9, 3, '2023-03-24 23:44:48', 'Patricia.Artajo', 'Patricia.Artajo', 'Patricia', 'Artajo', 'Patricia.Artajo@example.com', 0, 1, 1, 1, 1, '0000-00-00 00:00:00', 'Artajo, Patricia'),
(10, 4, '2023-03-24 23:47:08', 'sierrakx.tl', 'sierrakx.tl', 'Katherine', 'Masangkay', 'katherine.sierra@spi-global.com', 0, 0, 0, 1, 0, '0000-00-00 00:00:00', 'Masangkay, Katherine'),
(11, 6, '2023-03-24 23:47:50', 'sierrakxstp.tl', 'sierrakxstp.tl', 'Kaye', 'Sierra', 'katherine.sierra@spi-global.com', 0, 0, 0, 0, 0, '0000-00-00 00:00:00', 'Sierra, Kaye'),
(12, 7, '2023-03-24 23:48:34', 'sierrakx.stp1', 'sierrakx.stp1', 'ka', 'masangkay', 'katherine.sierra@spi-global.com', 0, 0, 0, 0, 0, '0000-00-00 00:00:00', 'masangkay, ka'),
(13, 7, '2023-03-24 23:49:15', 'sierrakx.stp2', 'sierrakx.stp2', 'ka', 'sierra', 'katherine.sierra@spi-global.com', 0, 0, 0, 0, 0, '0000-00-00 00:00:00', 'sierra, ka'),
(14, 7, '2023-03-24 23:50:05', 'sierrakx.stp3', 'sierrakx.stp3', 'katherine', 'sierra', 'katherine.sierra@spi-global.com', 0, 0, 0, 0, 0, '0000-00-00 00:00:00', 'sierra, katherine'),
(15, 3, '2023-03-29 05:03:49', 'Chelsea.Mercado', 'Chelsea.Mercado', 'Chelsea', 'Mercado', 'Chelsea.Mercado@example.com', 0, 1, 1, 1, 1, '0000-00-00 00:00:00', 'Mercado, Chelsea'),
(16, 5, '2023-04-15 23:54:02', 'sierrak.coding3', 'sierrak.coding3', 'Kaye', 'Sierra', 'katherine.sierra@spi-global.com', 0, 0, 0, 0, 0, '0000-00-00 00:00:00', 'Sierra, Kaye'),
(17, 5, '2023-04-15 23:54:56', 'sierrakx.coding2', 'sierrakx.coding2', 'Kat', 'Sierra', 'katherine.sierra@spi-global.com', 0, 0, 0, 0, 0, '0000-00-00 00:00:00', 'Sierra, Kat'),
(18, 5, '2023-04-15 23:55:57', 'sierrakx.coding', 'sierrakx.coding', 'kaye', 'masangkay', 'katherine.sierra@spi-global.com', 0, 0, 0, 0, 0, '0000-00-00 00:00:00', 'masangkay, kaye'),
(19, 5, '2023-04-15 23:56:50', 'Coding1', 'Coding1', 'Coding1', 'Coding1', 'Coding1@example.com', 0, 0, 1, 0, 1, '0000-00-00 00:00:00', 'Coding1, Coding1'),
(20, 5, '2023-04-15 23:57:36', 'Coding2', 'Coding2', 'Coding2', 'Coding2', 'Coding2@example.com', 0, 0, 1, 0, 1, '0000-00-00 00:00:00', 'Coding2, Coding2');
...
--
-- Table structure for table `historytrail`
--

CREATE TABLE `historytrail` (
  `ID` int(11) NOT NULL,
...
CREATE TABLE `jobcoversheetdata` (
  ...
  `TargetPressDate` datetime DEFAULT NULL,
  `ActualPressDate` datetime DEFAULT NULL,
  `LatestTaskNumber` int(11) DEFAULT NULL,
...
CREATE TABLE `manuscriptquery` (
  ...
  `QueryID` int(11) DEFAULT NULL,
  `querystatus_mt_ID` int(11) DEFAULT NULL,
  `querytopic_mt_ID` int(11) DEFAULT NULL,
  `querytype_mt_ID` int(11) DEFAULT NULL,
  `FilePath` varchar(1000) CHARACTER SET utf8 COLLATE utf8_general_ci DEFAULT NULL,
  `IsReplied` bit(1) NOT NULL DEFAULT b'0',
  `DatePosted` datetime DEFAULT NULL,
  `PostedBy` varchar(45) DEFAULT NULL,
  `ClosedDate` datetime DEFAULT NULL
...
--
-- Table structure for table `notification_posts`
--

CREATE TABLE `notification_posts` (
  `ID` int(11) NOT NULL,
  `QueryID` int(11) DEFAULT NULL,
  `QueryReplyID` int(11) DEFAULT NULL,
  `NotificationQueryTypeID` int(11) DEFAULT NULL,
  `DateCreated` datetime DEFAULT NULL,
  `CreatedBy` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;
...
--
-- Table structure for table `notification_querytype`
--

CREATE TABLE `notification_querytype` (
  `ID` int(11) NOT NULL,
  `Description` varchar(128) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `notification_querytype`
--

INSERT INTO `notification_querytype` (`ID`, `Description`) VALUES
(1, 'Manuscript'),
(2, 'Coversheet'),
(3, 'SendToPrint');
...
--
-- Table structure for table `notification_useraccess`
--

CREATE TABLE `notification_useraccess` (
  `ID` int(11) NOT NULL,
  `NotificationQueryTypeID` int(11) DEFAULT NULL,
...
--
-- Dumping data for table `notification_useraccess`
--

INSERT INTO `notification_useraccess` (`ID`, `NotificationQueryTypeID`, `UserAccessID`) VALUES
(1, 1, 1),
(2, 1, 2),
(3, 1, 3),
(12, 2, 1),
...
--
-- Table structure for table `notification_userrecord`
--

CREATE TABLE `notification_userrecord` (
  `ID` int(11) NOT NULL,
  `NotificationPostID` int(11) DEFAULT NULL,
...
--
-- Dumping data for table `querytopic_mt`
--

INSERT INTO `querytopic_mt` (`ID`, `TopicTitle`) VALUES
(1, 'Topic 1'),
(2, 'Topic 2'),
(3, 'Topic 3'),
(4, 'Topic 4');
...
--
-- Table structure for table `querytype_mt`
--

CREATE TABLE `querytype_mt` (
  `ID` int(11) NOT NULL,
  `Type` varchar(45) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;

--
-- Dumping data for table `querytype_mt`
--

INSERT INTO `querytype_mt` (`ID`, `Type`) VALUES
(1, 'General'),
(2, 'XML Tagging'),
(3, 'Format');
...
CREATE TABLE `stp` (
  `ID` int(11) NOT NULL,
  `SendToPrintNumber` varchar(200) DEFAULT NULL,
  `OwnerUserID` int(11) DEFAULT NULL,
  `CoversheetIDs` varchar(128) DEFAULT NULL,
  `PathOfInputFiles` varchar(100) DEFAULT NULL,
  `IsConsoHighlight` smallint(6) DEFAULT 1,
  ...
  `IsSendingFinalPages` smallint(6) DEFAULT 1,
  ...
  `SendToPrintTier` varchar(45) DEFAULT NULL,
  `BPSProductID` varchar(128) DEFAULT NULL,
  `ServiceNumber` varchar(128) DEFAULT NULL,
  `CurrentTask` varchar(250) DEFAULT NULL,
  `SendToPrintStatus` varchar(128) DEFAULT NULL,
  `TargetPressDate` datetime DEFAULT NULL,
  `ActualPressDate` datetime DEFAULT NULL,
  `CreatedEmployeeID` int(11) DEFAULT NULL,
  `DateCreated` datetime DEFAULT NULL,
...
CREATE TABLE `stpquery` (
  `ID` int(11) NOT NULL,
  `STPNo` varchar(200) DEFAULT NULL,
  `QueryID` int(11) DEFAULT NULL,
  `StpID` int(11) DEFAULT NULL,
  `stptopic_mt_ID` int(11) DEFAULT NULL,
  `stpstatus_mt_ID` int(11) DEFAULT NULL,
  `stptype_mt_ID` int(11) DEFAULT NULL,
  `FilePath` varchar(1000) DEFAULT NULL,
  `IsReplied` bit(1) NOT NULL DEFAULT b'0',
  `DatePosted` datetime DEFAULT NULL,
  `PostedBy` varchar(128) DEFAULT NULL,
...
--
-- Table structure for table `stpreplies`
--

CREATE TABLE `stpreplies` (
  `ID` int(11) NOT NULL,
  `QueryID` int(11) DEFAULT NULL,
  `Message` varchar(500) DEFAULT NULL,
  `DatePosted` datetime DEFAULT NULL,
...
--
-- Table structure for table `stpstatus_mt`
--

CREATE TABLE `stpstatus_mt` (
  `ID` int(11) NOT NULL,
  `Status` varchar(128) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;

--
-- Dumping data for table `stpstatus_mt`
--

INSERT INTO `stpstatus_mt` (`ID`, `Status`) VALUES
(1, 'Open'),
(2, 'Cancelled'),
(3, 'Closed');
...
--
-- Table structure for table `stptopic_mt`
--

CREATE TABLE `stptopic_mt` (
  `ID` int(11) NOT NULL,
  `TopicTitle` varchar(128) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;

--
-- Dumping data for table `stptopic_mt`
--

INSERT INTO `stptopic_mt` (`ID`, `TopicTitle`) VALUES
(1, 'Topic 1'),
(2, 'Topic 2'),
(3, 'Topic 3');
...
--
-- Table structure for table `stptype_mt`
--

CREATE TABLE `stptype_mt` (
  `ID` int(11) NOT NULL,
  `TYPE` varchar(128) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;

--
-- Dumping data for table `stptype_mt`
--

INSERT INTO `stptype_mt` (`ID`, `TYPE`) VALUES
(1, 'General'),
(2, 'XML Tagging'),
(3, 'Format');
...
(11, 'TOC', 'COMMENTARY', 0, 5, 0, 2, 7, 0),
(12, 'TOS', 'COMMENTARY', 0, 5, 0, 2, 7, 0),
(13, 'Other', 'COMMENTARY', 0, 5, 0, 2, 7, 0);
...
INSERT INTO `useraccess_mt` (`ID`, `UserAccess`) VALUES
(1, 'Admin'),
(2, 'Client(LE)'),
(3, 'Straive(PE)'),
(4, 'Coding TL'),
...
--
-- Indexes for table `coverreplies`
--
...
--
-- Indexes for table `coverstatus_mt`
--
ALTER TABLE `coverstatus_mt`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `covertopic_mt`
--
ALTER TABLE `covertopic_mt`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `covertype_mt`
--
ALTER TABLE `covertype_mt`
  ADD PRIMARY KEY (`ID`);
...
--
-- Indexes for table `historytrail`
--
ALTER TABLE `historytrail`
  ADD PRIMARY KEY (`ID`);
...
--
-- Indexes for table `notification_posts`
--
ALTER TABLE `notification_posts`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `notification_querytype`
--
ALTER TABLE `notification_querytype`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `notification_useraccess`
--
ALTER TABLE `notification_useraccess`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `notification_userrecord`
--
ALTER TABLE `notification_userrecord`
  ADD PRIMARY KEY (`ID`);
...
--
-- Indexes for table `querytype_mt`
--
ALTER TABLE `querytype_mt`
  ADD PRIMARY KEY (`ID`);
...
--
-- Indexes for table `stpreplies`
--
ALTER TABLE `stpreplies`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `stpstatus_mt`
--
ALTER TABLE `stpstatus_mt`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `stptopic_mt`
--
ALTER TABLE `stptopic_mt`
  ADD PRIMARY KEY (`ID`);

--
-- Indexes for table `stptype_mt`
--
ALTER TABLE `stptype_mt`
  ADD PRIMARY KEY (`ID`);
...
--
ALTER TABLE `coversheetdata`
  MODIFY `CoversheetID` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `coverstatus_mt`
--
ALTER TABLE `coverstatus_mt`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT for table `covertopic_mt`
--
ALTER TABLE `covertopic_mt`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT for table `covertype_mt`
--
ALTER TABLE `covertype_mt`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
-- AUTO_INCREMENT for table `historytrail`
--
ALTER TABLE `historytrail`
...
--
ALTER TABLE `jobcoversheetdata`
  MODIFY `JobCoversheetID` int(11) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `manuscriptdata`
  MODIFY `ManuscriptID` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `manuscriptquery`
--
ALTER TABLE `manuscriptquery`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `notification_posts`
--
ALTER TABLE `notification_posts`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `notification_querytype`
--
ALTER TABLE `notification_querytype`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT for table `notification_useraccess`
--
ALTER TABLE `notification_useraccess`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=19;

--
-- AUTO_INCREMENT for table `notification_userrecord`
--
ALTER TABLE `notification_userrecord`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `querytype_mt`
--
ALTER TABLE `querytype_mt`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
...
--
-- AUTO_INCREMENT for table `stptopic_mt`
--
ALTER TABLE `stptopic_mt`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;

--
-- AUTO_INCREMENT for table `stptype_mt`
--
ALTER TABLE `stptype_mt`
  MODIFY `ID` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
