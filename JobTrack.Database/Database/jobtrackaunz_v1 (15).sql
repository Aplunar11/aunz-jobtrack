...
--
-- Host: 127.0.0.1
-- Generation Time: May 09, 2023 at 03:29 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateReplyCoversheet` (`p_id` INT, `p_query_id` INT, `p_message` NVARCHAR(250), `p_user` NVARCHAR(128))   BEGIN
	DECLARE p_isNew BIT;
	DECLARE p_employeeID INT;    
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateReplyManuscript` (`p_id` INT, `p_query_id` INT, `p_message` NVARCHAR(250), `p_user` NVARCHAR(128))   BEGIN
	DECLARE p_isNew BIT;
	DECLARE p_employeeID INT;    
    SELECT ID INTO p_employeeID FROM employee WHERE UserName = p_user;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `UpdateReplySTP` (`p_id` INT, `p_query_id` INT, `p_message` NVARCHAR(250), `p_user` NVARCHAR(128))   BEGIN
	DECLARE p_isNew BIT;
	DECLARE p_employeeID INT;    
    SELECT ID INTO p_employeeID FROM employee WHERE UserName = p_user;
